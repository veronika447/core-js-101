...
function angleBetweenClockHands(date) {
  ...
  const degreeHours = hours * 30 + minutes * 0.5;
  const degreeMinutes = minutes * 6;
  let degree;
  if ((degreeHours >= degreeMinutes)) { degree = degreeHours - degreeMinutes; }
  if (degreeHours < degreeMinutes) { degree = degreeMinutes - degreeHours; }
  if (degree > 180) { degree = 360 - degree; }
  return Math.PI * ((degree * 2) / 360);
}
